...
# -----------------------------------------
# 3) MAPA DE ETAPAS → URLs
# -----------------------------------------

BASE = (
    "https://api.sif-source.org/projects/{guid}/questions/search/{code}"
    "?SPHostUrl=https%3A%2F%2Fwww.sif-source.org"
    "&SPLanguage=pt-BR"
    "&SPClientTag=0"
...
ETAPAS = {
    # Etapas existentes
    "Estudos": 2001218,             # etapa2
    "Consulta Pública": 2001221,    # etapa5
    "TCU": 2001226,                 # etapa9
    "Edital": 2001229,              # etapa13
    "Data do Contrato": 2001231,    # contrato - data de assinatura
    "Situação do Contrato": 2001230, # contrato - situação
    
    # Novas etapas
    "Data Início Licitação": 2001227,        # etapa12
    "Data Fim Licitação": 2001228,           # etapa11
    "Status Fase Licitação": 2001229,        # etapa13
    "Status Consulta Pública": 2001221,      # etapa5 (já existia)
    "Status Estudos": 2001218,               # etapa2 (já existia)
    "Data Início Consulta Pública": 2001219, # etapa3
    "Data Fim Controle Externo": 2001223,    # etapa7
    "Questões Chaves": 2000727,              # QuestoesChaves
    "Status": 2000726,                       # Status atual do projeto (adicionado conforme solicitação)
    "Data Início Controle Externo": 2001222, # etapa6
    "Data Fim Estudos": 2001217,             # etapa1
    "Próximas Etapas": 2000728,              # Próximas Etapas
    "Data Fim Consulta Pública": 2001220,    # etapa4
    "Status Controle Externo": 2001224,      # etapa8
    "Status Aviso Licitação": 2001226,       # etapa9 (já existia como TCU)
    "Data Publicação Aviso Licitação": 2001225, # FaseStatus
    "Data Início Estudos": 2001216           # etapa
}

# -----------------------------------------
# 4) FUNÇÃO PARA BAIXAR STATUS
# -----------------------------------------

def fetch_status(guid, etapa, code):
    url = BASE.format(guid=guid, code=code)

    try:
        r = requests.get(url, timeout=15, auth=AUTH)

        if r.status_code != 200:
            return etapa, f"Erro HTTP {r.status_code}"

        data = r.json()

        # Caso exista resposta válida
        if "value" in data and len(data["value"]) > 0:
            txt = data["value"][0].get("text", "").strip()
            return etapa, txt if txt else "Vazio"

        return etapa, "Sem dados"

    except Exception as e:
        return etapa, f"Erro: {type(e).__name__}"

# -----------------------------------------
# 5) PROCESSA UM PROJETO COMPLETO
# -----------------------------------------

def process_project(guid):
    row = {"guid": guid}

    with ThreadPoolExecutor(max_workers=4) as pool:
        futures = [
            pool.submit(fetch_status, guid, etapa, code)
            for etapa, code in ETAPAS.items()
        ]

        for f in as_completed(futures):
            etapa, status = f.result()
            row[etapa] = status
...
def main():
    print("🔄 Baixando informações dos projetos...")
    start_time = time.time()

    results = []
    with ThreadPoolExecutor(max_workers=20) as executor:
        futures = {executor.submit(process_project, g): g for g in GUIDS}
    ...
    print(f"\n⏱️ Finalizado em {elapsed:.2f} segundos.\n")

    # JSON
    with open("projetos_status.json", "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=2)

    print("📄 Arquivo salvo → projetos_status.json")

    # CSV
    df = pd.DataFrame(results)
    df.to_csv("projetos_status.csv", index=False, encoding="utf-8")
    print("📄 Arquivo salvo → projetos_status.csv")
